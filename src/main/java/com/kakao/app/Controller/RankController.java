...
public class RankController {
    ...
    public ResponseEntity<UserEntity> addRank(@RequestBody RankRequest rankRequest) {
        ...
        Optional<UserEntity> userEntity = userRepository.findByUserId(rankRequest.getUserId()); //rankRequest에서 id만 확인해서 그 레코드를 가져옴(원래 있던 레코드)

        //DB에 기존 userId가 없는 경우
        if (!userEntity.isPresent()) {
            UserEntity savedUser = rankService.save(rankRequest);
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(savedUser);
        } else { //DB에 기존 userId가 있는 경우
